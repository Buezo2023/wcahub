...
export function BISection({ showToast }) {
  ...
  return(
    <div>
      ...
      {history.length>0&&(
        <div style={{background:"var(--bg-surface)",border:"1px solid var(--border)",borderRadius:14,padding:20,marginBottom:16}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
            {history.map(h=>(
              <div key={h.month} style={{fontSize:10,color:"var(--text-tertiary)",textAlign:"center"}}>
      ...
      {stats?.byProgram&&Object.keys(stats.byProgram).length>0&&(
        <div style={{background:"var(--bg-surface)",border:"1px solid var(--border)",borderRadius:14,padding:20,marginBottom:16}}>
          ...
          {Object.entries(stats.byProgram).map(([prog,count])=>{
            ...
            return(
              <div key={prog} style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
      ...
      {/* Full BI link */}
      <div style={{background:PD,border:`1px solid ${P}20`,borderRadius:12,padding:"14px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:10}}>
        <div>
        ...
        <button onClick={()=>navigate("/bi")} style={{padding:"8px 18px",background:P,color:"#fff",border:"none",borderRadius:9,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
          Abrir BI Dashboard →
        </button>
      </div>
      </>}
