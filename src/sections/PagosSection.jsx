    ...
    <div>
              ...
              {filtered.map(p=>{
                ...
                return (
                  <tr key={p.id} style={{borderTop:"1px solid var(--border-tertiary)"}}>
                    ...
                    <td style={{padding:"10px 12px"}}><span style={{fontSize:10,padding:"2px 8px",borderRadius:12,background:bg,color,fontWeight:600}}>{statusLabel(p.status)}</span></td>
      ...
      {regModal&&(
        <div style={{position:"fixed",inset:0,zIndex:10000,background:"rgba(0,0,0,.45)",display:"flex",alignItems:"center",justifyContent:"center",padding:16}}
          ...
          <div style={{background:"var(--bg-surface)",borderRadius:18,padding:24,width:"min(420px, calc(100vw - 32px))",maxWidth:"100%",width:"100%",border:"1px solid var(--border)",boxShadow:"0 20px 60px rgba(0,0,0,.15)",maxHeight:"90vh",overflowY:"auto"}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setRegModal(false)} style={{flex:1,padding:"10px",background:"var(--bg-surface-subtle)",border:"1px solid var(--border)",borderRadius:9,fontSize:13,cursor:"pointer",fontFamily:"inherit",color:"var(--text-secondary)"}}>Cancelar</button>
              <button onClickCapture={e=>{e.stopPropagation();register();}} style={{flex:2,padding:"10px",background:P,color:"#fff",border:"none",borderRadius:9,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",opacity:saving?.6:1}}>
