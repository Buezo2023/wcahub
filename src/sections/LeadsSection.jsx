                ...
                <div style={{display:"flex",flexDirection:"column",gap:8}}>
                  {stagLeads.map(lead=>(
                    <div key={lead.id} onClick={()=>setSel(sel?.id===lead.id?null:lead)}
                      ...
                      <div style={{fontSize:11,color:"var(--text-secondary)",marginTop:2}}>{lead.email}</div>
                      {lead.test_score>0&&<div style={{fontSize:11,marginTop:4,color:lead.test_score>=70?"#059669":"#d97706",fontWeight:600}}>Test: {lead.test_score}%</div>}
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div style={{marginTop:12,display:"flex",flexDirection:"column",gap:8}}>
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
              {STAGES.filter(s=>s.id!==sel.stage&&s.id!=="convertido").map(s=>(
                <button key={s.id} onClick={()=>changeStage(sel.id,s.id)} style={{fontSize:11,padding:"3px 8px",borderRadius:8,border:`1px solid ${s.color}`,background:s.bg,color:s.color,cursor:"pointer",fontFamily:"inherit",fontWeight:600}}>{s.label}</button>
              ))}
            </div>
            {sel.stage!=="convertido"&&sel.stage!=="perdido"&&(
              <button onClickCapture={e=>{e.stopPropagation();convertLead(sel);}} style={{marginTop:4,padding:"8px",background:GD,color:G,border:`1px solid ${G}40`,borderRadius:8,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
                🎉 Convertir y matricular
              </button>
            )}
            <button onClickCapture={e=>{e.stopPropagation();changeStage(sel.id,"perdido");setSel(null);}} style={{padding:"8px",background:RD,color:R,border:"none",borderRadius:8,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
              Marcar como perdido
            </button>
          </div>
        </div>
          ...
          <div style={{background:"var(--bg-surface)",borderRadius:16,padding:24,width:"min(400px, calc(100vw - 32px))",border:"1px solid var(--border)",boxShadow:"0 20px 60px rgba(0,0,0,.15)"}}>
            ...
            <div style={{marginBottom:18}}>
              ...
              <textarea value={form.notes} onChange={e=>setForm(p=>({...p,notes:e.target.value}))} rows={2}
                style={{width:"100%",padding:"10px 13px",border:"1px solid var(--border)",borderRadius:8,fontSize:13,background:"var(--bg-surface-subtle)",color:"var(--text-primary)",fontFamily:"inherit",resize:"none"}}/>
            </div>
            <div style={{display:"flex",gap:8}}>
              ...
              <button onClickCapture={e=>{e.stopPropagation();addLead();}} style={{flex:2,padding:"10px",background:P,color:"#fff",border:"none",borderRadius:8,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",opacity:saving?.6:1}}>
