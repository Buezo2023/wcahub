...
function Badge({text,bg,color}){
  return <span style={{fontSize:11,padding:"2px 8px",borderRadius:12,background:bg,color,fontWeight:600,whiteSpace:"nowrap"}}>{text}</span>;
}
    ...
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar estudiante..."
          style={{flex:1,minWidth:160,padding:"9px 13px",border:"1px solid var(--border)",borderRadius:8,fontSize:13,background:"var(--bg-surface)",color:"var(--text-primary)",fontFamily:"inherit"}}/>
      ...
      <div style={{display:"flex",gap:16}}>
        {/* Table */}
        <div style={{flex:1,background:"var(--bg-surface)",border:"1px solid var(--border)",borderRadius:12,overflow:"hidden",minWidth:0}}>
          {loading ? (
        ...
        {/* Detail panel */}
        {sel && (
          <div style={{width:260,flexShrink:0,background:"var(--bg-surface)",border:"1px solid var(--border)",borderRadius:12,padding:16,height:"fit-content"}}>
            ...
            <div style={{fontSize:11,color:"var(--text-secondary)",textAlign:"center",marginBottom:16}}>{sel.profile?.email||"—"}</div>
            {[
              ["Nivel", sel.level||"—"],
            ...
            <button onClickCapture={e=>{e.stopPropagation();toggleActive(sel);}}
              style={{width:"100%",marginTop:16,padding:"9px",background:sel.profile?.active!==false?RD:GD,color:sel.profile?.active!==false?R:G,border:"none",borderRadius:8,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
          ...
          <div style={{background:"var(--bg-surface)",borderRadius:16,padding:26,width:"min(440px, calc(100vw - 32px))",border:"1px solid var(--border)",boxShadow:"0 20px 60px rgba(0,0,0,.15)",maxHeight:"90vh",overflowY:"auto"}}>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setEnrollModal(false)}
                style={{flex:1,padding:"10px",background:"var(--bg-surface-subtle)",border:"1px solid var(--border)",borderRadius:8,fontSize:13,cursor:"pointer",fontFamily:"inherit",color:"var(--text-secondary)"}}>Cancelar</button>
              <button onClickCapture={e=>{e.stopPropagation();enroll();}} disabled={enrolling}
                style={{flex:2,padding:"10px",background:P,color:"#fff",border:"none",borderRadius:8,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",opacity:enrolling?.6:1}}>
