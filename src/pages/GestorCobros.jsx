    ...
    <div style={{ display:"flex", height:720, background:B.bg, borderRadius:16, overflow:"hidden", border:`1px solid ${B.border}`, fontFamily:"'DM Sans','Segoe UI',sans-serif", position:"relative" }}>
      ...
      <aside style={{ width:196, background:B.primary, display:"flex", flexDirection:"column", padding:"0 0 14px", flexShrink:0 }}>
        <div style={{ padding:"18px 16px 16px", borderBottom:"1px solid rgba(255,255,255,.1)", marginBottom:8 }}>
          ...
          <div style={{ fontSize:9, color:"rgba(255,255,255,.4)", marginTop:2, letterSpacing:1, textTransform:"uppercase" }}>Gestor de Cobros</div>
        </div>
        {NAV.map(item => (
          <button key={item.id} onClick={() => { setView(item.id); setSelTransfer(null); }} style={{
            ...
            {item.id==="pending" && pending.length > 0 && (
              <span style={{ marginLeft:"auto", fontSize:9, background:B.secondary, color:B.dark, borderRadius:"50%", width:16, height:16, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700 }}>{pending.length}</span>
            )}
            {item.id==="overdue" && OVERDUE.length > 0 && (
              <span style={{ marginLeft:"auto", fontSize:9, background:B.red, color:"#fff", borderRadius:"50%", width:16, height:16, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700 }}>{OVERDUE.length}</span>
            )}
          </button>
        ))}
        <div style={{ marginTop:"auto", padding:"12px 16px 0", borderTop:"1px solid rgba(255,255,255,.08)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            <div style={{ width:28, height:28, borderRadius:"50%", background:"rgba(255,255,255,.15)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:700, color:B.secondary }}>GC</div>
            <div><div style={{ fontSize:10, color:"#fff", fontWeight:600 }}>Gestor WCA</div><div style={{ fontSize:9, color:"rgba(255,255,255,.4)" }}>Cobros</div></div>
          </div>
        </div>
      </aside>
      ...
      <main style={{ flex:1, overflow:"hidden", display:"flex", flexDirection:"column" }}>
        <div style={{ height:52, background:B.white, borderBottom:`1px solid ${B.border}`, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 20px", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", gap:8 }}>
            {pending.length > 0 && <div style={{ fontSize:10, background:B.amberDim, color:"#92400e", padding:"3px 10px", borderRadius:20, fontWeight:600 }}>⏳ {pending.length} por confirmar</div>}
            {OVERDUE.length > 0 && <div style={{ fontSize:10, background:B.redDim, color:B.red, padding:"3px 10px", borderRadius:20, fontWeight:600 }}>⚠ {OVERDUE.length} vencidos</div>}
          </div>
        </div>
        ...
        <div style={{ flex:1, overflow:"auto", padding:18 }}>
            ...
            <div>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:14 }}>
                  <div style={{ fontSize:12, fontWeight:700, color:B.text, marginBottom:10 }}>Transferencias pendientes hoy</div>
                  {pending.length === 0 && <div style={{ fontSize:11, color:B.textSec, textAlign:"center", padding:"16px 0" }}>✓ Todo confirmado</div>}
                  {pending.map(t => (
                    <div key={t.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 10px", background:t.urgent?B.redDim:B.amberDim, borderRadius:9, marginBottom:7, border:`1px solid ${t.urgent?B.red:B.amber}40` }}>
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:11, fontWeight:600, color:B.text }}>{t.student}</div>
                      ...
                      <div style={{ display:"flex", gap:5 }}>
                        {t.proof && <button onClick={() => setProofModal(t)} style={{ fontSize:9, padding:"4px 8px", background:B.white, color:B.textSec, border:`1px solid ${B.border}`, borderRadius:5, cursor:"pointer", fontFamily:"inherit" }}>Ver</button>}
                        <button onClick={() => confirmTransfer(t.id)} style={{ fontSize:9, padding:"4px 9px", background:B.green, color:"#fff", border:"none", borderRadius:5, cursor:"pointer", fontWeight:600, fontFamily:"inherit" }}>✓ OK</button>
                      </div>
                    </div>
                  ))}
                </div>
                <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:14 }}>
                  <div style={{ fontSize:12, fontWeight:700, color:B.text, marginBottom:10 }}>Últimas acciones</div>
                  {AUDIT_LOG.slice(0,5).map((a,i) => (
                    <div key={i} style={{ display:"flex", gap:9, padding:"7px 0", borderTop:i>0?`1px solid ${B.borderLight}`:"none" }}>
                      ...
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:11, color:B.text }}><strong>{a.action}</strong> — {a.student}</div>
                        <div style={{ fontSize:9, color:B.textSec }}>{a.amount} · {a.time}</div>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{ flex:1, overflow:"auto" }}>
                {pending.length === 0 && (
                  <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:40, textAlign:"center" }}>
                    ...
                    <div style={{ fontSize:14, fontWeight:700, color:B.text, marginBottom:4 }}>Todo confirmado</div>
                    <div style={{ fontSize:12, color:B.textSec }}>No hay transferencias pendientes en este momento.</div>
                  </div>
                )}
                {pending.map(t => (
                  <div key={t.id} onClick={() => setSelTransfer(selTransfer?.id===t.id?null:t)}
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                      <div>
                        <div style={{ fontSize:13, fontWeight:700, color:B.text }}>{t.student}</div>
                        <div style={{ fontSize:10, color:B.textSec, marginTop:2 }}>{t.level} · {t.program} · {t.date}</div>
                      </div>
                    ...
                    <div style={{ display:"flex", gap:7 }}>
                      {t.proof && <button onClick={e=>{e.stopPropagation();setProofModal(t);}} style={{ flex:1, fontSize:10, padding:"7px", background:B.bg, color:B.textSec, border:`1px solid ${B.border}`, borderRadius:8, cursor:"pointer", fontFamily:"inherit" }}>Ver comprobante</button>}
                      <button onClick={e=>{e.stopPropagation();setRejectModal(t);}} style={{ flex:1, fontSize:10, padding:"7px", background:B.redDim, color:B.red, border:`1px solid ${B.red}40`, borderRadius:8, cursor:"pointer", fontWeight:600, fontFamily:"inherit" }}>✗ Rechazar</button>
                      <button onClick={e=>{e.stopPropagation();confirmTransfer(t.id);}} style={{ flex:2, fontSize:10, padding:"7px", background:B.green, color:"#fff", border:"none", borderRadius:8, cursor:"pointer", fontWeight:600, fontFamily:"inherit" }}>✓ Confirmar pago</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div style={{ maxWidth:520 }}>
              {regSuccess ? (
                <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:32, textAlign:"center" }}>
                  ...
                  <div style={{ fontSize:12, color:B.textSec, marginBottom:20 }}>El acceso del estudiante ha sido activado y el recibo fue generado automáticamente.</div>
                  <button onClick={() => { setRegSuccess(false); setRegForm({ student:"", amount:"", method:"Efectivo", bank:"", note:"", code:"" }); }} style={{ padding:"9px 20px", background:B.primary, color:"#fff", border:"none", borderRadius:9, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Registrar otro pago</button>
                </div>
              ) : (
                <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:20 }}>
                  <div style={{ fontSize:13, fontWeight:700, color:B.text, marginBottom:16 }}>Registrar pago manual</div>
                  {[
                    ...
                    <div key={f.key} style={{ marginBottom:12 }}>
                      ...
                      <input type={f.type} value={registerForm[f.key]} onChange={e=>setRegForm(r=>({...r,[f.key]:e.target.value}))} placeholder={f.ph} style={{ width:"100%", padding:"9px 12px", border:`1px solid ${B.border}`, borderRadius:9, fontSize:12, background:B.bg, fontFamily:"inherit", color:B.text }} />
                    </div>
                  ))}
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:12 }}>
                    <div>
                      <label style={{ fontSize:11, color:B.textSec, display:"block", marginBottom:4 }}>Método</label>
                      <select value={registerForm.method} onChange={e=>setRegForm(r=>({...r,method:e.target.value}))} style={{ width:"100%", padding:"9px 10px", border:`1px solid ${B.border}`, borderRadius:9, fontSize:11, background:B.bg, fontFamily:"inherit" }}>
                        <option>Efectivo</option><option>Transferencia</option><option>Tarjeta</option>
                      </select>
                    </div>
                    {registerForm.method === "Transferencia" && (
                      <div>
                        <label style={{ fontSize:11, color:B.textSec, display:"block", marginBottom:4 }}>Banco</label>
                        <select value={registerForm.bank} onChange={e=>setRegForm(r=>({...r,bank:e.target.value}))} style={{ width:"100%", padding:"9px 10px", border:`1px solid ${B.border}`, borderRadius:9, fontSize:11, background:B.bg, fontFamily:"inherit" }}>
                  ...
                  <div style={{ marginBottom:12 }}>
                    <label style={{ fontSize:11, color:B.textSec, display:"block", marginBottom:4 }}>Nota interna (visible en auditoría)</label>
                    <input value={registerForm.note} onChange={e=>setRegForm(r=>({...r,note:e.target.value}))} placeholder="Ej: Pagó en sede, entregó comprobante físico..." style={{ width:"100%", padding:"9px 12px", border:`1px solid ${B.border}`, borderRadius:9, fontSize:12, background:B.bg, fontFamily:"inherit", color:B.text }} />
                  </div>
            ...
            <div>
              <div style={{ display:"flex", gap:8, marginBottom:12 }}>
                <div style={{ flex:1, display:"flex", alignItems:"center", gap:8, background:B.white, border:`1px solid ${B.border}`, borderRadius:9, padding:"7px 12px" }}>
                  ...
                  <input value={searchHist} onChange={e=>setSearchHist(e.target.value)} placeholder="Buscar por estudiante o código..." style={{ border:"none", outline:"none", fontSize:12, background:"transparent", flex:1, fontFamily:"inherit", color:B.text }} />
                </div>
                <button style={{ padding:"7px 14px", background:B.white, border:`1px solid ${B.border}`, borderRadius:9, fontSize:11, cursor:"pointer", color:B.textSec, fontFamily:"inherit" }}>↓ Excel</button>
              </div>
                      ...
                      return (
                        <tr key={p.id} style={{ borderTop:`1px solid ${B.borderLight}` }}>
                          <td style={{ padding:"10px 10px" }}>
                            ...
                            <div style={{ fontSize:9, color:B.textSec }}>{p.date}</div>
                          </td>
                          ...
                          <td style={{ padding:"10px 10px" }}><code style={{ fontSize:9, background:B.bg, padding:"2px 6px", borderRadius:4, color:B.textSec }}>{p.code}</code></td>
                          ...
                          <td style={{ padding:"10px 10px" }}>
                            {(p.status==="confirmed" && p.action_by==="Gestor") && (
                              <button onClick={() => setAnulaModal(p)} style={{ fontSize:9, padding:"3px 8px", background:B.redDim, color:B.red, border:"none", borderRadius:5, cursor:"pointer", fontFamily:"inherit" }}>Anular</button>
                            )}
                          </td>
                        </tr>
                      );
          ...
          {view==="overdue" && (
            <div>
              <div style={{ background:B.redDim, border:`1px solid ${B.red}40`, borderRadius:10, padding:"10px 14px", marginBottom:14, fontSize:11, color:B.red, display:"flex", gap:8 }}>
                <i className="ti ti-alert-circle" style={{ fontSize:13, flexShrink:0, marginTop:1 }} aria-hidden="true" />
              ...
              {OVERDUE.map((o,i) => (
                <div key={i} style={{ background:B.white, border:`1px solid ${B.red}40`, borderRadius:12, padding:16, marginBottom:10 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                    <div>
                      <div style={{ fontSize:14, fontWeight:700, color:B.text }}>{o.student}</div>
                      <div style={{ fontSize:11, color:B.textSec }}>{o.level} · <span style={{ color:B.red, fontWeight:600 }}>Vencido hace {o.days} días</span></div>
                    </div>
                    <div style={{ textAlign:"right" }}>
                  ...
                  <div style={{ display:"flex", gap:7 }}>
                    <button style={{ flex:1, fontSize:10, padding:"8px", background:B.bg, color:B.textSec, border:`1px solid ${B.border}`, borderRadius:8, cursor:"pointer", fontFamily:"inherit" }}>
                      <i className="ti ti-brand-whatsapp" style={{ fontSize:12, verticalAlign:-1, marginRight:4 }} aria-hidden="true" />
                      WhatsApp: {o.contact}
                    </button>
                    <button onClick={() => setView("register")} style={{ flex:1, fontSize:10, padding:"8px", background:B.primary, color:"#fff", border:"none", borderRadius:8, cursor:"pointer", fontWeight:600, fontFamily:"inherit" }}>
                      Registrar pago
                    </button>
                  </div>
                </div>
                  ...
                  <div key={i} style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:14 }}>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
                      <div>
                        ...
                        <div style={{ fontSize:10, color:B.textSec }}>{p.date} · {p.method}</div>
                      </div>
                      <div style={{ fontSize:18, fontWeight:800, color:B.primary }}>${p.amount}</div>
                    </div>
                    <div style={{ display:"flex", gap:6 }}>
                      <button style={{ flex:1, fontSize:10, padding:"6px", background:B.primaryDim, color:B.primary, border:"none", borderRadius:7, cursor:"pointer", fontFamily:"inherit" }}>↓ PDF</button>
                      <button style={{ flex:1, fontSize:10, padding:"6px", background:B.bg, color:B.textSec, border:`1px solid ${B.border}`, borderRadius:7, cursor:"pointer", fontFamily:"inherit" }}>Reenviar</button>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {view==="audit" && (
            <div>
              <div style={{ background:B.primaryDim, border:`1px solid ${B.border}`, borderRadius:10, padding:"9px 14px", marginBottom:12, fontSize:11, color:B.primary, display:"flex", gap:8 }}>
                <i className="ti ti-shield-check" style={{ fontSize:13, flexShrink:0 }} aria-hidden="true" />
              ...
              <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, overflow:"hidden" }}>
                {AUDIT_LOG.map((a,i) => (
                  <div key={i} style={{ display:"flex", gap:12, padding:"11px 14px", borderBottom:i<AUDIT_LOG.length-1?`1px solid ${B.borderLight}`:"none" }}>
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:12, color:B.text }}><strong>{a.action}</strong> — {a.student} · <span style={{ fontWeight:700, color:B.primary }}>{a.amount}</span></div>
                      <div style={{ fontSize:10, color:B.textSec, marginTop:2 }}>{a.note}</div>
                    </div>
                    <div style={{ fontSize:10, color:B.textSec, whiteSpace:"nowrap" }}>{a.time}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      {proofModal && (
        <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,.5)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:50, borderRadius:16 }}>
          <div style={{ background:B.white, borderRadius:16, padding:24, width:380, border:`1px solid ${B.border}` }}>
            <div style={{ fontSize:14, fontWeight:700, color:B.text, marginBottom:4 }}>Comprobante de transferencia</div>
            <div style={{ fontSize:11, color:B.textSec, marginBottom:14 }}>{proofModal.student} · ${proofModal.amount} · {proofModal.code}</div>
            <div style={{ background:B.bg, borderRadius:10, height:160, display:"flex", alignItems:"center", justifyContent:"center", marginBottom:14, border:`1px solid ${B.border}` }}>
              <div style={{ textAlign:"center", color:B.textSec }}>
                ...
                <div style={{ fontSize:11 }}>Imagen del comprobante</div>
                <div style={{ fontSize:10 }}>Banco: {proofModal.bank}</div>
              </div>
            </div>
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={() => setProofModal(null)} style={{ flex:1, padding:"9px", background:B.bg, color:B.textSec, border:`1px solid ${B.border}`, borderRadius:9, fontSize:11, cursor:"pointer", fontFamily:"inherit" }}>Cerrar</button>
              <button onClick={() => { confirmTransfer(proofModal.id); setProofModal(null); }} style={{ flex:2, padding:"9px", background:B.green, color:"#fff", border:"none", borderRadius:9, fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>✓ Confirmar pago</button>
            </div>
          </div>
        </div>
      )}
      ...
      {anulaModal && (
        <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,.5)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:50, borderRadius:16 }}>
          <div style={{ background:B.white, borderRadius:16, padding:24, width:380 }}>
            <div style={{ fontSize:14, fontWeight:700, color:B.text, marginBottom:4 }}>Anular pago</div>
            <div style={{ fontSize:11, color:B.textSec, marginBottom:14 }}>{anulaModal.student} · ${anulaModal.amount} — esta acción queda en el log de auditoría.</div>
            <label style={{ fontSize:11, color:B.textSec, display:"block", marginBottom:5 }}>Motivo de la anulación (obligatorio)</label>
            <input value={anulaNote} onChange={e=>setAnulaNote(e.target.value)} placeholder="Ej: Error en el monto, reembolso solicitado..." style={{ width:"100%", padding:"9px 12px", border:`1px solid ${B.border}`, borderRadius:8, fontSize:11, background:B.bg, fontFamily:"inherit", marginBottom:14 }} />
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={() => { setAnulaModal(null); setAnulaNote(""); }} style={{ flex:1, padding:"9px", background:B.bg, border:`1px solid ${B.border}`, borderRadius:9, fontSize:11, cursor:"pointer", fontFamily:"inherit", color:B.textSec }}>Cancelar</button>
              <button onClick={() => { setAnulaModal(null); setAnulaNote(""); }} style={{ flex:1, padding:"9px", background:anulaNote?B.red:B.border, color:anulaNote?"#fff":B.textSec, border:"none", borderRadius:9, fontSize:11, fontWeight:600, cursor:anulaNote?"pointer":"not-allowed", fontFamily:"inherit" }}>Anular pago</button>
            </div>
          </div>
        </div>
      )}
