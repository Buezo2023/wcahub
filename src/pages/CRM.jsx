...
function Tag({ label }) {
  ...
  return <span style={{ fontSize: 9, padding: "2px 7px", borderRadius: 20, background: bg, color: col, fontWeight: 600, letterSpacing: 0.3 }}>{label}</span>;
}

function ScoreBadge({ score }) {
  return (
    <div style={{ display:"flex", alignItems:"center", gap: 5 }}>
      <div style={{ width: 32, height: 32, borderRadius: "50%", background: scoreBg(score), display:"flex", alignItems:"center", justifyContent:"center", fontSize: 11, fontWeight: 700, color: scoreColor(score), flexShrink: 0 }}>{score}</div>
    ...
    <div onClick={onClick} style={{
      ...
      <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom: 8 }}>
        <div style={{ display:"flex", alignItems:"center", gap: 8 }}>
          <div style={{ width: 34, height: 34, borderRadius:"50%", background: stage.bg, display:"flex", alignItems:"center", justifyContent:"center", fontSize: 12, fontWeight: 700, color: stage.color, flexShrink:0 }}>
            {lead.name.split(" ").map(n=>n[0]).join("").slice(0,2)}
          </div>
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: selected ? "#1f2933" : "#1f2933", lineHeight: 1.2 }}>{lead.name}</div>
            <div style={{ fontSize: 10, color: selected ? "#6b7280" : "#6b7280", marginTop: 2 }}>{lead.country} · {lead.source}</div>
          </div>
        </div>
        <ScoreBadge score={lead.score} />
      </div>
      <div style={{ fontSize: 10, color: selected ? "#6b7280" : "#6b7280", marginBottom: 6, fontStyle:"italic", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>"{lead.lastMsg}"</div>
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div style={{ display:"flex", gap: 4, flexWrap:"wrap" }}>
          <span style={{ fontSize: 9, padding:"2px 8px", borderRadius:20, background: stage.bg, color: stage.color, fontWeight:600 }}>{stage.label}</span>
          {lead.tags.map(t => <Tag key={t} label={t} />)}
        </div>
        <div style={{ fontSize: 9, color: selected ? "#6b7280" : "#6b7280" }}>{lead.date}</div>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{ height:"100%", display:"flex", flexDirection:"column", overflow:"hidden" }}>
      ...
      <div style={{ padding:"16px 20px 12px", borderBottom:"1px solid #d1dde3", flexShrink:0 }}>
        <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", marginBottom:10 }}>
          <div style={{ display:"flex", gap:12, alignItems:"center" }}>
            <div style={{ width:44, height:44, borderRadius:"50%", background: stage.bg, display:"flex", alignItems:"center", justifyContent:"center", fontSize:15, fontWeight:700, color: stage.color }}>
              {lead.name.split(" ").map(n=>n[0]).join("").slice(0,2)}
            </div>
            <div>
              <div style={{ fontSize:15, fontWeight:700, color:"#1f2933" }}>{lead.name}</div>
              <div style={{ fontSize:11, color:"#6b7280", marginTop:2 }}>{lead.country} · {lead.email}</div>
            </div>
          </div>
          <button onClick={onClose} style={{ background:"#1f2933", border:"none", borderRadius:8, width:28, height:28, cursor:"pointer", fontSize:14, color:"#6b7280", display:"flex", alignItems:"center", justifyContent:"center" }}>✕</button>
        </div>
        {/* Stage pills */}
        <div style={{ display:"flex", gap:6, flexWrap:"wrap", alignItems:"center" }}>
          {STAGES.map(s => (
            <span key={s.id} style={{ fontSize:9, padding:"3px 9px", borderRadius:20, background: s.id === lead.stage ? s.color : s.bg, color: s.id === lead.stage ? "#fff" : s.color, fontWeight:600, cursor:"pointer", transition:"all .15s" }}>{s.label}</span>
          ))}
        </div>
      </div>
      ...
      <div style={{ display:"flex", borderBottom:"1px solid #d1dde3", flexShrink:0 }}>
        {tabs.map(t => (
          <button key={t.id} onClick={() => setTab(t.id)} style={{ flex:1, padding:"9px 4px", border:"none", background:"transparent", fontSize:11, fontWeight:500, color: tab===t.id ? "#1f2933" : "#6b7280", borderBottom: tab===t.id ? "2px solid #1f2933" : "2px solid transparent", cursor:"pointer", transition:"all .15s" }}>{t.label}</button>
        ))}
      </div>
      ...
      <div style={{ flex:1, overflow:"auto", padding:"14px 18px" }}>
        ...
        {tab === "info" && (
          <div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:14 }}>
              {[
                ...
                <div key={label} style={{ background:"#1f2933", borderRadius:8, padding:"8px 10px" }}>
                  ...
                  <div style={{ fontSize:11, color:"#1f2933", fontWeight:500 }}>{val}</div>
                </div>
              ))}
            </div>
            ...
            <div style={{ background:"#1f2933", borderRadius:10, padding:"12px 14px", marginBottom:12 }}>
              <div style={{ fontSize:10, color:"#6b7280", marginBottom:8, fontWeight:600, letterSpacing:.5, textTransform:"uppercase" }}>Lead Score</div>
            ...
            {/* Actions */}
            <div style={{ fontSize:10, color:"#6b7280", fontWeight:600, letterSpacing:.5, textTransform:"uppercase", marginBottom:8 }}>Acciones rápidas</div>
            <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
              {!lead.level && !sentTest && (
                <button onClick={() => setSentTest(true)} style={{ padding:"9px 14px", background:"#155266", color:"#fff", border:"none", borderRadius:10, fontSize:11, fontWeight:600, cursor:"pointer", textAlign:"left", transition:"opacity .15s" }}>
                  📋 Enviar Placement Test
                </button>
              )}
              {sentTest && (
                <div style={{ padding:"9px 14px", background:"#e8f3f6", borderRadius:10, fontSize:11, color:"#155266", fontWeight:500 }}>✓ Link enviado — esperando resultado</div>
              )}
              {lead.level && (
                <div style={{ padding:"9px 14px", background:"#d1fae5", borderRadius:10, fontSize:11, color:"#059669", fontWeight:500 }}>✓ Nivel detectado: {lead.level} — listo para propuesta</div>
              )}
              <button style={{ padding:"9px 14px", background:"#1f2933", color:"#fff", border:"none", borderRadius:10, fontSize:11, fontWeight:600, cursor:"pointer", textAlign:"left" }}>
                💬 Enviar mensaje WhatsApp
              </button>
              {lead.stage === "propuesta" && (
                <button style={{ padding:"9px 14px", background:"#059669", color:"#fff", border:"none", borderRadius:10, fontSize:11, fontWeight:600, cursor:"pointer", textAlign:"left" }}>
                  🎉 Marcar como convertido
                </button>
              )}
              <button style={{ padding:"9px 14px", background:"transparent", color:"#dc2626", border:"1px solid #fecaca", borderRadius:10, fontSize:11, fontWeight:600, cursor:"pointer", textAlign:"left" }}>
                ❌ Marcar como perdido
              </button>
            </div>
          </div>
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:0, position:"relative" }}>
            ...
            {lead.activity.map((a, i) => (
              <div key={i} style={{ display:"flex", gap:10, padding:"8px 0", position:"relative", zIndex:1 }}>
                <div style={{ width:24, height:24, borderRadius:"50%", background:"#1f2933", border:"1.5px solid #d1dde3", display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, flexShrink:0 }}>{actIcon(a.type)}</div>
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:11, color:"#1f2933", lineHeight:1.5 }}>{a.text}</div>
        ...
        {tab === "notas" && (
          <div>
            <div style={{ fontSize:10, color:"#6b7280", fontWeight:600, letterSpacing:.5, textTransform:"uppercase", marginBottom:8 }}>Notas del asesor</div>
            <textarea value={note} onChange={e => setNote(e.target.value)}
              style={{ width:"100%", minHeight:120, border:"1px solid #d1dde3", borderRadius:10, padding:"10px 12px", fontSize:11, color:"#1f2933", lineHeight:1.7, background:"#1f2933", resize:"vertical", fontFamily:"inherit" }}
              placeholder="Añade notas sobre este lead..." />
            <button style={{ marginTop:8, padding:"8px 16px", background:"#1f2933", color:"#fff", border:"none", borderRadius:8, fontSize:11, fontWeight:600, cursor:"pointer" }}>Guardar nota</button>

            <div style={{ fontSize:10, color:"#6b7280", fontWeight:600, letterSpacing:.5, textTransform:"uppercase", margin:"16px 0 8px" }}>Agregar recordatorio</div>
            <div style={{ display:"flex", gap:8 }}>
              <input type="text" value={msg} onChange={e=>setMsg(e.target.value)} placeholder="Ej: Llamar mañana a las 10am" style={{ flex:1, padding:"8px 10px", border:"1px solid #d1dde3", borderRadius:8, fontSize:11, background:"#1f2933", fontFamily:"inherit" }} />
              <button onClick={()=>setMsg("")} style={{ padding:"8px 14px", background:"#155266", color:"#fff", border:"none", borderRadius:8, fontSize:11, fontWeight:600, cursor:"pointer" }}>+ Tarea</button>
            </div>
          </div>
        )}
      </div>
    ...
    <div style={{ display:"flex", height:680, background:"#1f2933", borderRadius:16, overflow:"hidden", border:"1px solid #d1dde3", fontFamily:"'DM Sans','Outfit','Segoe UI',sans-serif" }}>
      ...
      <div style={{ width:200, background:"#1f2933", display:"flex", flexDirection:"column", padding:"18px 0", flexShrink:0 }}>
        <div style={{ padding:"0 18px 16px", borderBottom:"1px solid rgba(255,255,255,.08)", marginBottom:8 }}>
          ...
          <div style={{ fontSize:9, color:"rgba(255,255,255,.3)", marginTop:2, letterSpacing:1, textTransform:"uppercase" }}>Ventas · Enrollment</div>
        </div>
        ...
        {[
          ...
          <button key={item.id} onClick={() => setView(item.id)} style={{
            display:"flex", alignItems:"center", gap:9, padding:"9px 18px", border:"none", background: view===item.id ? "rgba(124,58,237,.2)" : "transparent",
            color: view===item.id ? "#155266" : "rgba(255,255,255,.4)", fontSize:12, cursor:"pointer", textAlign:"left",
        ...
        <div style={{ marginTop:"auto", padding:"14px 18px", borderTop:"1px solid rgba(255,255,255,.08)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:8 }}>
            <div style={{ width:28, height:28, borderRadius:"50%", background:"#e8f3f6", display:"flex", alignItems:"center", justifyContent:"center", fontSize:10, fontWeight:700, color:"#155266" }}>VN</div>
            <div>
              <div style={{ fontSize:11, color:"#1f2933", fontWeight:500 }}>Asesor WCA</div>
              <div style={{ fontSize:9, color:"rgba(255,255,255,.3)" }}>Ventas</div>
            </div>
          </div>
        </div>
      </div>
      ...
      <div style={{ flex:1, overflow:"hidden", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ padding:"0 20px", height:52, background:"#fff", borderBottom:"1px solid #d1dde3", display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ fontSize:10, background:"#e8f3f6", color:"#155266", padding:"3px 10px", borderRadius:20, fontWeight:600 }}>{LEADS.filter(l=>l.stage!=="convertido"&&l.stage!=="perdido").length} activos</div>
            <div style={{ fontSize:10, background:"#d1fae5", color:"#059669", padding:"3px 10px", borderRadius:20, fontWeight:600 }}>{convRate}% conv.</div>
            <button style={{ padding:"6px 14px", background:"#1f2933", color:"#fff", border:"none", borderRadius:8, fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>+ Nuevo lead</button>
          </div>
        </div>
                  ...
                  <div key={stage.id} style={{ background:"#1f2933", borderRadius:12, padding:"12px 12px 6px" }}>
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                        ...
                        <span style={{ fontSize:11, fontWeight:700, color:"#1f2933" }}>{stage.label}</span>
                      </div>
                      <span style={{ fontSize:10, fontWeight:700, color: stage.color, background: stage.bg, padding:"1px 8px", borderRadius:20 }}>{byStage(stage.id).length}</span>
                    </div>
                    {byStage(stage.id).length === 0 && (
                      <div style={{ textAlign:"center", padding:"20px 0", fontSize:10, color:"#c5d5db" }}>Sin leads</div>
                    )}
          ...
          <div style={{ flex:1, overflow:"hidden", display:"flex" }}>
            <div style={{ flex:1, overflow:"auto", padding:"16px 18px" }}>
              <div style={{ display:"flex", gap:8, marginBottom:14 }}>
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar lead..." style={{ flex:1, padding:"8px 12px", border:"1px solid #d1dde3", borderRadius:10, fontSize:12, background:"#1f2933", fontFamily:"inherit" }} />
                <select value={filterStage} onChange={e=>setFilterStage(e.target.value)} style={{ padding:"8px 10px", border:"1px solid #d1dde3", borderRadius:10, fontSize:11, background:"#1f2933", fontFamily:"inherit" }}>
                  ...
                  <thead>
                    <tr style={{ background:"#1f2933" }}>
                      {["Lead","País","Fuente","Programa","Nivel","Score","Etapa",""].map(h => (
                        <th key={h} style={{ padding:"9px 10px", textAlign:"left", fontSize:9, fontWeight:700, color:"#6b7280", letterSpacing:.5, textTransform:"uppercase", borderBottom:"1px solid #d1dde3" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      return (
                        <tr key={l.id} onClick={() => { setSelected(l); }} style={{ borderBottom:"1px solid #1f2933", cursor:"pointer", background: selected?.id===l.id ? "#1f2933" : "transparent", transition:"background .1s" }}>
                          <td style={{ padding:"9px 10px" }}>
                            ...
                            <div style={{ fontSize:9, color:"#6b7280" }}>{l.email}</div>
                          </td>
                          ...
                          <td style={{ padding:"9px 10px" }}>{l.level ? <span style={{ background:"#e8f3f6", color:"#155266", padding:"2px 8px", borderRadius:20, fontWeight:600, fontSize:9 }}>{l.level}</span> : <span style={{ color:"#c5d5db" }}>—</span>}</td>
                          <td style={{ padding:"9px 10px" }}>
                            <span style={{ fontSize:11, fontWeight:700, color: scoreColor(l.score) }}>{l.score}</span>
                          </td>
                          <td style={{ padding:"9px 10px" }}>
                            <span style={{ fontSize:9, padding:"2px 8px", borderRadius:20, background: st.bg, color: st.color, fontWeight:600 }}>{st.label}</span>
                          </td>
                          <td style={{ padding:"9px 10px" }}>
                            <button onClick={e=>{e.stopPropagation();setSelected(l);}} style={{ fontSize:9, padding:"3px 9px", background:"#1f2933", border:"none", borderRadius:6, cursor:"pointer", fontFamily:"inherit", color:"#6b7280" }}>Ver →</button>
                          </td>
                        </tr>
                      );
                    })}
        ...
        {view === "tareas" && (
          <div style={{ flex:1, overflow:"auto", padding:"18px 20px" }}>
            ...
            <div style={{ background:"#fff", borderRadius:12, border:"1px solid #d1dde3", padding:"14px 16px" }}>
              <div style={{ fontSize:11, fontWeight:700, color:"#6b7280", letterSpacing:.5, textTransform:"uppercase", marginBottom:12 }}>Pendientes hoy</div>
              {tasks.map((t, i) => (
                <div key={t.id} style={{ display:"flex", alignItems:"flex-start", gap:12, padding:"10px 0", borderTop: i > 0 ? "1px solid #1f2933" : "none" }}>
                  ...
                  <div style={{ flex:1 }}>
                    <div style={{ fontSize:12, fontWeight:500, color:"#1f2933", marginBottom:2 }}>{t.text}</div>
                    <div style={{ fontSize:10, color:"#6b7280" }}>Lead: <strong>{t.lead}</strong> · {t.due}</div>
                  </div>
                  <div style={{ display:"flex", gap:5 }}>
                    <button onClick={()=>setTasks(ts=>ts.filter(x=>x.id!==t.id))} style={{ fontSize:10, padding:"4px 10px", background:"#d1fae5", color:"#059669", border:"none", borderRadius:6, cursor:"pointer", fontWeight:600, fontFamily:"inherit" }}>✓ Hecho</button>
                    <button style={{ fontSize:10, padding:"4px 10px", background:"#1f2933", color:"#6b7280", border:"none", borderRadius:6, cursor:"pointer", fontFamily:"inherit" }}>Posponer</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ flex:1, overflow:"auto", padding:"18px 20px" }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
              <div style={{ background:"#fff", border:"1px solid #d1dde3", borderRadius:12, padding:"14px 16px" }}>
                <div style={{ fontSize:11, fontWeight:700, color:"#1f2933", marginBottom:12 }}>Leads por etapa</div>
                {STAGES.map(s => {
                  ...
                  return (
                    <div key={s.id} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:7 }}>
                      <span style={{ fontSize:10, color:"#6b7280", width:80 }}>{s.label}</span>
                      ...
                      <span style={{ fontSize:10, fontWeight:700, color: s.color, width:16, textAlign:"right" }}>{n}</span>
                    </div>
                  );
                })}
              </div>
              ...
              <div style={{ background:"#fff", border:"1px solid #d1dde3", borderRadius:12, padding:"14px 16px" }}>
                <div style={{ fontSize:11, fontWeight:700, color:"#1f2933", marginBottom:12 }}>Fuentes de leads</div>
                {SOURCE_DATA.map(s => (
                  <div key={s.label} style={{ display:"flex", alignItems:"center", gap:8, marginBottom:7 }}>
                    ...
                    <span style={{ fontSize:10, fontWeight:700, color:"#155266", width:24, textAlign:"right" }}>{s.count}</span>
                  </div>
                ))}
              </div>
            </div>
