    ...
    <div style={{ display:"flex", height:720, background:B.bg, borderRadius:16, overflow:"hidden", border:`1px solid ${B.border}`, fontFamily:"'DM Sans','Segoe UI',sans-serif" }}>
      ...
      <aside style={{ width:200, background:B.primary, display:"flex", flexDirection:"column", padding:"0 0 16px", flexShrink:0 }}>
        <div style={{ padding:"20px 18px 18px", borderBottom:"1px solid rgba(255,255,255,.1)", marginBottom:10 }}>
          ...
          <div style={{ fontSize:9, color:"rgba(255,255,255,.4)", marginTop:2, letterSpacing:1, textTransform:"uppercase" }}>Panel Admin</div>
        </div>
        ...
        {NAV.map(item => (
          <button key={item.id} onClick={() => { setView(item.id); setSelStudent(null); }} style={{
            ...
            <i className={`ti ${item.icon}`} style={{ fontSize:15, width:18, textAlign:"center" }} aria-hidden="true" />
            {item.label}
            {item.id==="payments" && PAYMENTS_PENDING.length > 0 && (
              <span style={{ marginLeft:"auto", fontSize:9, background:B.secondary, color:B.primary, borderRadius:"50%", width:16, height:16, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700 }}>{PAYMENTS_PENDING.length}</span>
            )}
          </button>
        ))}
        ...
        <div style={{ marginTop:"auto", padding:"14px 18px 0", borderTop:"1px solid rgba(255,255,255,.1)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:9 }}>
            <div style={{ width:32, height:32, borderRadius:"50%", background:"rgba(255,255,255,.15)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:12, fontWeight:700, color:B.secondary }}>AD</div>
            <div>
              <div style={{ fontSize:11, color:"#fff", fontWeight:600 }}>Admin WCA</div>
              <div style={{ fontSize:9, color:"rgba(255,255,255,.4)" }}>Administrador</div>
            </div>
          </div>
        </div>
      </aside>
      ...
      <main style={{ flex:1, overflow:"hidden", display:"flex", flexDirection:"column" }}>
        ...
        <div style={{ height:54, background:B.white, borderBottom:`1px solid ${B.border}`, display:"flex", alignItems:"center", justifyContent:"space-between", padding:"0 22px", flexShrink:0 }}>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center" }}>
            {suspended.length > 0 && <div style={{ fontSize:10, background:B.redDim, color:B.red, padding:"3px 10px", borderRadius:20, fontWeight:600 }}>⚠ {suspended.length} suspendidos</div>}
            <div style={{ fontSize:10, background:B.primaryDim, color:B.primary, padding:"3px 10px", borderRadius:20, fontWeight:600 }}>{active.length} activos hoy</div>
            {view === "students" && (
              <button onClick={() => setEnrollModal(true)} style={{ padding:"6px 14px", background:B.primary, color:"#fff", border:"none", borderRadius:8, fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit", display:"flex", alignItems:"center", gap:5 }}>
                <i className="ti ti-user-plus" style={{ fontSize:13 }} aria-hidden="true" /> Nuevo estudiante
              </button>
            )}
          </div>
        </div>
        ...
        <div style={{ flex:1, overflow:"auto", padding:20 }}>
            ...
            <div>
              ...
              {ALERTS.map((a, i) => (
                <div key={i} style={{ background: a.type==="red"?B.redDim:a.type==="amber"?B.amberDim:B.primaryDim, border:`1px solid ${a.type==="red"?B.red:a.type==="amber"?B.amber:B.border}`, borderRadius:10, padding:"9px 14px", marginBottom:7, display:"flex", alignItems:"center", gap:10 }}>
                  <i className={`ti ${a.icon}`} style={{ fontSize:15, color:a.type==="red"?B.red:a.type==="amber"?"#92400e":B.primary, flexShrink:0 }} aria-hidden="true" />
                  <div style={{ flex:1, fontSize:11, color:B.text }}>{a.text}</div>
                  {a.action && <button onClick={() => setView(a.action==="Ver estudiantes"?"students":a.action==="Ver pagos"?"payments":"groups")} style={{ fontSize:10, padding:"4px 10px", background:B.white, border:`1px solid ${B.border}`, borderRadius:6, cursor:"pointer", color:B.primary, fontWeight:600, fontFamily:"inherit", whiteSpace:"nowrap" }}>{a.action} →</button>}
                </div>
              ))}
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                ...
                <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16 }}>
                  <div style={{ fontSize:12, fontWeight:700, color:B.text, marginBottom:12, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                    Estado del ciclo — esta semana
                    <button onClick={() => setView("cycle")} style={{ fontSize:9, padding:"3px 8px", background:B.primaryDim, color:B.primary, border:"none", borderRadius:6, cursor:"pointer", fontFamily:"inherit" }}>Ver todo</button>
                  </div>
                  {CYCLE_STATUS.map(c => (
                    <div key={c.level} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 0", borderTop:`1px solid ${B.borderLight}` }}>
                      <div style={{ width:32, height:32, borderRadius:8, background:B.primaryDim, display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:700, color:B.primary, flexShrink:0 }}>{c.level}</div>
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:11, fontWeight:600, color:B.text }}>U{c.unit} — {c.title}</div>
                        <div style={{ fontSize:10, color:B.textSec, marginTop:1 }}>{c.students} estudiantes · {c.groups} grupos</div>
                      </div>
                      ...
                      <div style={{ fontSize:10, color:B.textSec, width:22, textAlign:"right" }}>{c.unit}/12</div>
                    </div>
                  ))}
                </div>
                ...
                {/* Recent activity */}
                <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16 }}>
                  <div style={{ fontSize:12, fontWeight:700, color:B.text, marginBottom:12 }}>Actividad reciente</div>
                  {[
                    ...
                    <div key={i} style={{ display:"flex", gap:10, padding:"7px 0", borderTop: i>0?`1px solid ${B.borderLight}`:"none" }}>
                      <div style={{ width:28, height:28, borderRadius:"50%", background:B.bg, display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                        <i className={`ti ${a.icon}`} style={{ fontSize:14, color:a.color }} aria-hidden="true" />
                      </div>
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:11, color:B.text }}>{a.text}</div>
                        <div style={{ fontSize:10, color:B.textSec, marginTop:1 }}>{a.time}</div>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16 }}>
                  ...
                  {PAYMENTS_PENDING.map(p => (
                    <div key={p.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 10px", background:B.amberDim, borderRadius:8, marginBottom:7, border:`1px solid ${B.amber}40` }}>
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:11, fontWeight:600, color:B.text }}>{p.student}</div>
                        <div style={{ fontSize:10, color:B.textSec, marginTop:1 }}>{p.amount} · Código: {p.code} · {p.date}</div>
                      </div>
                      <button onClick={() => setView("payments")} style={{ fontSize:10, padding:"4px 10px", background:B.primary, color:"#fff", border:"none", borderRadius:6, cursor:"pointer", fontWeight:600, fontFamily:"inherit" }}>Confirmar</button>
                    </div>
                  ))}
                </div>
                  ...
                  {suspended.map(s => (
                    <div key={s.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 10px", background:B.redDim, borderRadius:8, marginBottom:7, border:`1px solid ${B.red}40` }}>
                      <div style={{ flex:1 }}>
                        ...
                        <div style={{ fontSize:10, color:B.textSec, marginTop:1 }}>{s.level} · {s.attendance}% asistencia</div>
                      </div>
                      <button onClick={() => { setView("students"); setSelStudent(s); }} style={{ fontSize:10, padding:"4px 10px", background:B.white, color:B.red, border:`1px solid ${B.red}`, borderRadius:6, cursor:"pointer", fontWeight:600, fontFamily:"inherit" }}>Ver</button>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ flex:1, overflow:"hidden", display:"flex", flexDirection:"column" }}>
                ...
                <div style={{ display:"flex", gap:8, marginBottom:12 }}>
                  <div style={{ flex:1, display:"flex", alignItems:"center", gap:8, background:B.white, border:`1px solid ${B.border}`, borderRadius:9, padding:"7px 12px" }}>
                    ...
                    <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar por nombre o email..." style={{ border:"none", outline:"none", fontSize:12, background:"transparent", color:B.text, flex:1, fontFamily:"inherit" }} />
                  </div>
                  <select value={filterState} onChange={e=>setFilterState(e.target.value)} style={{ padding:"7px 10px", border:`1px solid ${B.border}`, borderRadius:9, fontSize:11, background:B.white, color:B.text, fontFamily:"inherit" }}>
                    <option value="all">Todos los estados</option>
                    <option value="active">Activos</option>
                    <option value="suspended">Suspendidos</option>
                  </select>
                  <select value={filterLevel} onChange={e=>setFilterLevel(e.target.value)} style={{ padding:"7px 10px", border:`1px solid ${B.border}`, borderRadius:9, fontSize:11, background:B.white, color:B.text, fontFamily:"inherit" }}>
                    ...
                    <thead style={{ position:"sticky", top:0, zIndex:1 }}>
                      <tr style={{ background:B.bg, borderBottom:`1px solid ${B.border}` }}>
                        {["Estudiante","País","Nivel · Grupo","Programa","Tipo","Pago","Estado","Asist.",""].map(h => (
                          <th key={h} style={{ padding:"9px 10px", textAlign:"left", fontSize:9, fontWeight:600, color:B.textSec, letterSpacing:.5, textTransform:"uppercase", whiteSpace:"nowrap" }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        return (
                          <tr key={s.id} onClick={() => setSelStudent(selStudent?.id===s.id?null:s)} style={{ borderTop:`1px solid ${B.borderLight}`, cursor:"pointer", background: selStudent?.id===s.id ? B.primaryDim : "transparent", transition:"background .1s" }}>
                            <td style={{ padding:"10px 10px" }}>
                              ...
                              <div style={{ fontSize:9, color:B.textSec, marginTop:1 }}>{s.email}</div>
                            </td>
                            <td style={{ padding:"10px 10px", fontSize:13 }}>{s.country}</td>
                            <td style={{ padding:"10px 10px" }}>
                              <div style={{ fontWeight:500, color:B.text }}>{s.level}</div>
                              <div style={{ fontSize:9, color:B.textSec }}>{s.group}</div>
                            </td>
                            ...
                            <td style={{ padding:"10px 10px" }}><span style={{ fontSize:10, fontWeight:600, color: s.payment==="Al día"?B.green:s.payment==="Vencido"?B.red:B.textSec }}>{s.payment}</span></td>
                            ...
                            <td style={{ padding:"10px 10px" }}>
                              <button onClick={e=>{e.stopPropagation();setSelStudent(s);}} style={{ fontSize:9, padding:"3px 8px", background:B.primaryDim, color:B.primary, border:"none", borderRadius:5, cursor:"pointer", fontFamily:"inherit", fontWeight:600 }}>Ver →</button>
                            </td>
                          </tr>
                        );
                      })}
              ...
              {selStudent && (
                <div style={{ width:280, background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16, flexShrink:0, overflow:"auto" }}>
                  ...
                  {[
                    ...
                    <div key={k} style={{ display:"flex", justifyContent:"space-between", fontSize:11, padding:"6px 0", borderBottom:`1px solid ${B.borderLight}` }}>
                      <span style={{ color:B.textSec }}>{k}</span>
                      <span style={{ color:B.text, fontWeight:500 }}>{v}</span>
                    </div>
                  ))}
                  <div style={{ marginTop:12 }}>
                    <div style={{ fontSize:10, color:B.textSec, marginBottom:4 }}>Rendimiento</div>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:6, marginTop:12 }}>
                    {selStudent.state === "suspended" && (
                      <button style={{ padding:"8px", background:B.green, color:"#fff", border:"none", borderRadius:8, fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>✓ Reactivar cuenta</button>
                    )}
                    <button style={{ padding:"8px", background:B.primaryDim, color:B.primary, border:`1px solid ${B.border}`, borderRadius:8, fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Cambiar grupo/horario</button>
                    {selStudent.type === "scholarship" && (
                      <button style={{ padding:"8px", background:B.secondaryDim, color:"#92400e", border:`1px solid ${B.amber}40`, borderRadius:8, fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Upgrade a plan completo</button>
                    )}
                    <button style={{ padding:"8px", background:B.redDim, color:B.red, border:`1px solid ${B.red}40`, borderRadius:8, fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Suspender cuenta</button>
                  </div>
                </div>
              )}
            </div>
            ...
            <div>
              {GROUPS.filter(g=>!g.teamsSet).length > 0 && (
                <div style={{ background:B.redDim, border:`1px solid ${B.red}40`, borderRadius:10, padding:"10px 14px", marginBottom:14, display:"flex", gap:10, alignItems:"center" }}>
                  ...
                  <div style={{ flex:1, fontSize:11, color:B.text }}>
                ...
                {GROUPS.map(g => (
                  <div key={g.id} style={{ background:B.white, border:`1px solid ${g.teamsSet?B.border:B.red}`, borderRadius:12, padding:14 }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                      <div>
                        <div style={{ fontSize:15, fontWeight:800, color:B.primary }}>{g.level}</div>
                        <div style={{ fontSize:11, fontWeight:500, color:B.text }}>{g.time}</div>
                        <div style={{ fontSize:10, color:B.textSec }}>{g.days} · {g.teacher}</div>
                      </div>
                    ...
                    <div style={{ display:"flex", justifyContent:"space-between", fontSize:10, color:B.textSec, marginBottom:10 }}>
                      <span>{g.students}/{g.capacity} cupos</span>
                      <span>U{g.unit} activa</span>
                    </div>
                    {!g.teamsSet && (
                      <button onClick={() => setTeamsModal(g)} style={{ width:"100%", padding:"7px", background:B.primary, color:"#fff", border:"none", borderRadius:8, fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Configurar link Teams</button>
                    )}
                    {g.teamsSet && (
                      <button style={{ width:"100%", padding:"7px", background:B.primaryDim, color:B.primary, border:`1px solid ${B.border}`, borderRadius:8, fontSize:11, cursor:"pointer", fontFamily:"inherit" }}>Ver estudiantes del grupo</button>
                    )}
                  </div>
                ))}
              </div>
          ...
          {view === "payments" && (
            <div>
              ...
              <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16, marginBottom:14 }}>
                <div style={{ fontSize:12, fontWeight:700, color:B.text, marginBottom:12 }}>Transferencias por confirmar</div>
                {PAYMENTS_PENDING.map(p => (
                  <div key={p.id} style={{ display:"flex", alignItems:"center", gap:12, padding:"11px 12px", background:B.amberDim, borderRadius:10, marginBottom:8, border:`1px solid ${B.amber}40` }}>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:12, fontWeight:600, color:B.text }}>{p.student}</div>
                      <div style={{ fontSize:10, color:B.textSec, marginTop:2 }}>Código: <strong>{p.code}</strong> · {p.amount} · {p.date}</div>
                    </div>
                    <button style={{ fontSize:10, padding:"5px 12px", background:B.white, color:B.primary, border:`1px solid ${B.border}`, borderRadius:6, cursor:"pointer", fontWeight:500, fontFamily:"inherit" }}>Ver comprobante</button>
                    <button style={{ fontSize:10, padding:"5px 12px", background:B.green, color:"#fff", border:"none", borderRadius:6, cursor:"pointer", fontWeight:600, fontFamily:"inherit" }}>✓ Confirmar</button>
                  </div>
                ))}
              </div>
              <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16 }}>
                <div style={{ fontSize:12, fontWeight:700, color:B.text, marginBottom:12 }}>Historial reciente</div>
                {STUDENTS.slice(0,6).map((s,i) => (
                  <div key={s.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 0", borderTop:i>0?`1px solid ${B.borderLight}`:"none" }}>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:11, fontWeight:500, color:B.text }}>{s.name}</div>
                      <div style={{ fontSize:10, color:B.textSec }}>{s.program} · Mensual</div>
                    </div>
                    <div style={{ fontSize:12, fontWeight:600, color:B.text }}>$95</div>
          ...
          {view === "cycle" && (
            <div>
              <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16, marginBottom:14 }}>
                <div style={{ fontSize:12, fontWeight:700, color:B.text, marginBottom:14 }}>Estado del ciclo continuo — todos los niveles</div>
                {CYCLE_STATUS.map(c => (
                  <div key={c.level} style={{ marginBottom:16, paddingBottom:16, borderBottom:`1px solid ${B.borderLight}` }}>
                    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                        <div style={{ width:38, height:38, borderRadius:10, background:B.primaryDim, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:800, color:B.primary }}>{c.level}</div>
                        <div>
                          <div style={{ fontSize:12, fontWeight:600, color:B.text }}>U{c.unit} — {c.title}</div>
                          <div style={{ fontSize:10, color:B.textSec }}>{c.students} estudiantes · {c.groups} grupos activos</div>
                        </div>
                      </div>
                      <div style={{ textAlign:"right" }}>
                        <div style={{ fontSize:11, color:B.primary, fontWeight:600 }}>U{c.unit} / 12</div>
                        <div style={{ fontSize:9, color:B.textSec }}>Próximo avance: lunes</div>
                      </div>
                    </div>
              ...
              <div style={{ background:B.secondaryDim, border:`1px solid ${B.amber}40`, borderRadius:12, padding:14, display:"flex", gap:12, alignItems:"center" }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:12, fontWeight:600, color:B.text }}>Próximo feriado: 15 Sep — Independencia de Honduras</div>
                  <div style={{ fontSize:10, color:B.textSec, marginTop:2 }}>El ciclo se pausará automáticamente ese lunes. Configurado en Super Admin.</div>
                </div>
                <button style={{ fontSize:10, padding:"5px 12px", background:B.white, color:B.primary, border:`1px solid ${B.border}`, borderRadius:6, cursor:"pointer", fontWeight:600, fontFamily:"inherit", whiteSpace:"nowrap" }}>Ver calendario</button>
              </div>
            </div>
          )}
                ...
                {[
                  ...
                  <div key={i} style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:14, display:"flex", gap:12, alignItems:"flex-start", cursor:"pointer" }}>
                    ...
                    <div style={{ flex:1 }}>
                      ...
                      <div style={{ fontSize:10, color:B.textSec, lineHeight:1.5, marginBottom:8 }}>{r.desc}</div>
                      <div style={{ display:"flex", gap:6 }}>
                        <button style={{ fontSize:9, padding:"3px 10px", background:B.primaryDim, color:B.primary, border:"none", borderRadius:5, cursor:"pointer", fontWeight:600, fontFamily:"inherit" }}>Ver reporte</button>
                        <button style={{ fontSize:9, padding:"3px 10px", background:B.bg, color:B.textSec, border:`1px solid ${B.border}`, borderRadius:5, cursor:"pointer", fontFamily:"inherit" }}>↓ Exportar Excel</button>
                      </div>
                    </div>
                  </div>
                ))}
              ...
              {[
                ...
                <div key={i} style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16, marginBottom:10 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
                    <div>
                      <div style={{ fontSize:14, fontWeight:700, color:B.text }}>{co.name}</div>
                      <div style={{ fontSize:11, color:B.textSec }}>Contacto: {co.contact} · {co.employees} empleado{co.employees>1?"s":""}</div>
                    </div>
                    <div style={{ textAlign:"right" }}>
                      <div style={{ fontSize:13, fontWeight:700, color:B.primary }}>{co.amount}</div>
                      <Badge text="Activa" bg={B.greenDim} color="#065f46" />
                    </div>
                  </div>
                  <div style={{ display:"flex", gap:8 }}>
                    <button style={{ fontSize:10, padding:"5px 12px", background:B.primaryDim, color:B.primary, border:"none", borderRadius:6, cursor:"pointer", fontWeight:600, fontFamily:"inherit" }}>Ver empleados</button>
                    <button style={{ fontSize:10, padding:"5px 12px", background:B.bg, color:B.textSec, border:`1px solid ${B.border}`, borderRadius:6, cursor:"pointer", fontFamily:"inherit" }}>Generar factura</button>
                    <button style={{ fontSize:10, padding:"5px 12px", background:B.bg, color:B.textSec, border:`1px solid ${B.border}`, borderRadius:6, cursor:"pointer", fontFamily:"inherit" }}>Reportes progreso</button>
                  </div>
                </div>
              ))}
              <button style={{ width:"100%", padding:"11px", background:B.primary, color:"#fff", border:"none", borderRadius:10, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit", marginTop:4 }}>+ Crear cuenta empresa</button>
            </div>
          )}
          ...
          {view === "enrollments" && (
            <div style={{ maxWidth:560 }}>
              <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:20 }}>
                <div style={{ fontSize:13, fontWeight:700, color:B.text, marginBottom:16 }}>Nueva matrícula manual</div>
                {[
                  ...
                  <div key={f.label} style={{ marginBottom:12 }}>
                    ...
                    <input type={f.type} placeholder={f.ph} style={{ width:"100%", padding:"8px 12px", border:`1px solid ${B.border}`, borderRadius:8, fontSize:12, color:B.text, background:B.bg, fontFamily:"inherit" }} />
                  </div>
                ))}
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:12 }}>
                  <div>
                    <label style={{ fontSize:11, color:B.textSec, display:"block", marginBottom:4 }}>Programa</label>
                  ...
                  <div>
                    <label style={{ fontSize:11, color:B.textSec, display:"block", marginBottom:4 }}>Nivel de ingreso</label>
                    <select style={{ width:"100%", padding:"8px 10px", border:`1px solid ${B.border}`, borderRadius:8, fontSize:11, background:B.bg, fontFamily:"inherit" }}>
                  ...
                  <div>
                    <label style={{ fontSize:11, color:B.textSec, display:"block", marginBottom:4 }}>Forma de pago</label>
                    <select style={{ width:"100%", padding:"8px 10px", border:`1px solid ${B.border}`, borderRadius:8, fontSize:11, background:B.bg, fontFamily:"inherit" }}>
                      <option>Transferencia</option><option>Efectivo</option><option>Tarjeta (Stripe)</option>
                    </select>
                  </div>
                </div>
                <div style={{ background:B.primaryDim, borderRadius:8, padding:"10px 12px", fontSize:11, color:B.primary, marginBottom:16, display:"flex", gap:6 }}>
                ...
                <button style={{ width:"100%", padding:"11px", background:B.primary, color:"#fff", border:"none", borderRadius:10, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Crear matrícula</button>
              </div>
            </div>
          )}
      ...
      {teamsModal && (
        <div style={{ position:"absolute", inset:0, background:"rgba(0,0,0,.5)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:50, borderRadius:16 }}>
          <div style={{ background:B.white, borderRadius:16, padding:24, width:400, border:`1px solid ${B.border}` }}>
            <div style={{ fontSize:14, fontWeight:700, color:B.text, marginBottom:4 }}>Configurar link de Teams</div>
            <div style={{ fontSize:11, color:B.textSec, marginBottom:16 }}>Grupo: {teamsModal.level} · {teamsModal.time} · {teamsModal.days}</div>
            <label style={{ fontSize:11, color:B.textSec, display:"block", marginBottom:6 }}>Link de la reunión recurrente de Teams</label>
            <input value={teamsLink} onChange={e=>setTeamsLink(e.target.value)} placeholder="https://teams.microsoft.com/l/meetup-join/..." style={{ width:"100%", padding:"9px 12px", border:`1px solid ${B.border}`, borderRadius:8, fontSize:11, color:B.text, background:B.bg, fontFamily:"inherit", marginBottom:14 }} />
            <div style={{ background:B.primaryDim, borderRadius:8, padding:"9px 12px", fontSize:10, color:B.primary, marginBottom:16, display:"flex", gap:6 }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              <button onClick={() => setTeamsModal(null)} style={{ flex:1, padding:"9px", background:B.bg, color:B.textSec, border:`1px solid ${B.border}`, borderRadius:9, fontSize:11, cursor:"pointer", fontFamily:"inherit" }}>Cancelar</button>
              <button onClick={() => setTeamsModal(null)} style={{ flex:1, padding:"9px", background:B.primary, color:"#fff", border:"none", borderRadius:9, fontSize:11, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Guardar link</button>
            </div>
          </div>
        </div>
      )}
