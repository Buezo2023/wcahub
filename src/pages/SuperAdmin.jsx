...
function Pill({ text, color }) {
  return <span style={{ fontSize:12, padding:"3px 10px", borderRadius:20, background:`${color}18`, color, fontWeight:600, border:`0.5px solid ${color}40` }}>{text}</span>;
}
function SectionTitle({ children }) {
  return <div style={{ fontSize:12, fontWeight:600, color:B.textSec, textTransform:"uppercase", letterSpacing:.8, marginBottom:10, paddingBottom:8, borderBottom:`1px solid ${B.borderLight}` }}>{children}</div>;
}
      ...
      <aside style={{ width:196, background:B.dark, display:"flex", flexDirection:"column", padding:"0 0 14px", flexShrink:0 }}>
        ...
        {NAV.map(item => (
          <button key={item.id} onClick={() => setView(item.id)} style={{
            ...
            <i className={`ti ${item.icon}`} style={{ fontSize:15, width:16, textAlign:"center" }} aria-hidden="true" />
            {item.label}
          </button>
        ))}
        ...
        <div style={{ flex:1, overflow:"auto", padding:20 }}>
                ...
                {[
                  ...
                  <div key={i} style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:"14px 16px", borderTop:`3px solid ${s.color}` }}>
                    <div style={{ display:"flex", justifyContent:"space-between", marginBottom:8 }}>
                      <div style={{ fontSize:12, color:B.textSec }}>{s.label}</div>
                ...
                <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16 }}>
                  ...
                  {[
                    ...
                    <button key={i} onClick={() => setView(a.action)} style={{ width:"100%", display:"flex", alignItems:"center", gap:10, padding:"9px 10px", background:B.bg, border:`1px solid ${B.border}`, borderRadius:8, cursor:"pointer", marginBottom:6, textAlign:"left", fontFamily:"inherit" }}>
                      <i className={`ti ${a.icon}`} style={{ fontSize:15, color:B.primary, flexShrink:0 }} aria-hidden="true" />
                      <span style={{ fontSize:13, color:B.text }}>{a.label}</span>
                      <i className="ti ti-chevron-right" style={{ marginLeft:"auto", fontSize:13, color:B.textSec }} aria-hidden="true" />
                    </button>
                  ))}
                </div>
                <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16 }}>
                  ...
                  {AUDIT_LOG.slice(0,6).map((a,i) => (
                    <div key={i} style={{ display:"flex", gap:9, padding:"7px 0", borderBottom: i<5?`1px solid ${B.borderLight}`:"none" }}>
                      ...
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:13, color:B.text }}><strong>{a.user}</strong> — {a.action}</div>
                        <div style={{ fontSize:12, color:B.textSec, marginTop:1 }}>{a.detail}</div>
                      </div>
                      <div style={{ fontSize:11, color:B.textSec, whiteSpace:"nowrap" }}>{a.time}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {view === "prices" && (
            <div style={{ maxWidth:600 }}>
              <div style={{ background:B.amberDim, border:`1px solid ${B.amber}40`, borderRadius:10, padding:"10px 14px", marginBottom:16, fontSize:13, color:"#92400e", display:"flex", gap:8 }}>
              ...
              <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, overflow:"hidden" }}>
                <div style={{ padding:"12px 16px", borderBottom:`1px solid ${B.border}`, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                  <div style={{ fontSize:13, fontWeight:700, color:B.text }}>Planes y precios</div>
                ...
                {Object.entries(prices).map(([key, plan]) => (
                  <div key={key} style={{ display:"flex", alignItems:"center", gap:14, padding:"16px", borderBottom:`1px solid ${B.borderLight}` }}>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:14, fontWeight:600, color:B.text }}>{plan.label}</div>
                      <div style={{ fontSize:12, color:B.textSec, marginTop:2 }}>Cobro {plan.interval} · Stripe Subscriptions</div>
                    </div>
                    {editing === key ? (
                      ...
                      <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                        ...
                        <div style={{ fontSize:12, color:B.textSec }}>/{plan.interval==="trimestral"?"trim":"mes"}</div>
                        <button onClick={() => { setEditing(key); setTempPrice(String(plan.price)); }} style={{ padding:"5px 12px", background:B.primaryDim, color:B.primary, border:"none", borderRadius:7, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>✎ Editar</button>
                      </div>
                    )}
                  </div>
                ))}
              ...
              <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16, marginTop:12 }}>
                ...
                {[
                  ...
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:12, padding:"10px 0", borderBottom:i<1?`1px solid ${B.borderLight}`:"none" }}>
                    <code style={{ background:B.bg, padding:"4px 10px", borderRadius:6, fontSize:13, fontWeight:700, color:B.primary, border:`1px solid ${B.border}` }}>{c.code}</code>
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:13, color:B.text }}>Descuento: <strong>{c.disc}</strong> · Usos: {c.uses}</div>
                      <div style={{ fontSize:12, color:B.textSec }}>Vence: {c.expires}</div>
                    </div>
                    <button style={{ fontSize:12, padding:"4px 10px", background:B.redDim, color:B.red, border:"none", borderRadius:6, cursor:"pointer", fontFamily:"inherit" }}>Desactivar</button>
                  </div>
                ))}
                <button style={{ marginTop:10, width:"100%", padding:"8px", background:B.primaryDim, color:B.primary, border:`1px solid ${B.border}`, borderRadius:8, fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>+ Crear cupón</button>
              </div>
            </div>
          )}
              ...
              <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, overflow:"hidden" }}>
                ...
                {ROLES_LIST.map((r,i) => (
                  <div key={i} style={{ display:"flex", alignItems:"flex-start", gap:12, padding:"12px 16px", borderBottom:`1px solid ${B.borderLight}` }}>
                    <div style={{ width:36, height:36, borderRadius:"50%", background:B.primaryDim, display:"flex", alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:700, color:B.primary, flexShrink:0 }}>
                      {r.name.slice(0,2).toUpperCase()}
                    </div>
                    <div style={{ flex:1 }}>
                      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:4 }}>
                        <div style={{ fontSize:13, fontWeight:600, color:B.text }}>{r.name}</div>
                    ...
                    <div style={{ display:"flex", gap:5, flexShrink:0 }}>
                      <button style={{ fontSize:11, padding:"3px 8px", background:B.primaryDim, color:B.primary, border:"none", borderRadius:5, cursor:"pointer", fontFamily:"inherit" }}>Editar</button>
                      {r.name !== "Super Admin" && r.name !== "Estudiante" && (
                        <button style={{ fontSize:11, padding:"3px 8px", background:B.redDim, color:B.red, border:"none", borderRadius:5, cursor:"pointer", fontFamily:"inherit" }}>Eliminar</button>
                      )}
                    </div>
                  </div>
                ))}
          ...
          {view === "cycle" && (
            <div>
              <div style={{ background:B.primaryDim, border:`1px solid ${B.border}`, borderRadius:10, padding:"10px 14px", marginBottom:14, fontSize:13, color:B.primary, display:"flex", gap:8 }}>
                ...
                {CYCLE_LEVELS.map(c => (
                  <div key={c.level} style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:12 }}>
                      ...
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:15, fontWeight:700, color:B.text }}>Nivel {c.level} — Ciclo activo</div>
                        <div style={{ fontSize:13, color:B.textSec }}>U{c.unit}: {c.title} · {c.students} estudiantes · Iniciado: {c.started}</div>
                      </div>
                    ...
                    <div style={{ display:"flex", gap:8 }}>
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:12, color:B.textSec, marginBottom:4 }}>Mover a unidad específica</div>
                        <select style={{ padding:"6px 10px", border:`1px solid ${B.border}`, borderRadius:7, fontSize:13, background:B.bg, fontFamily:"inherit" }}>
                          {Array.from({length:12},(_,i) => <option key={i} selected={i+1===c.unit}>Unidad {i+1}</option>)}
                        </select>
                      </div>
                      <div style={{ display:"flex", gap:6, alignSelf:"flex-end" }}>
                        <button style={{ padding:"7px 14px", background:B.secondaryDim, color:"#92400e", border:`1px solid ${B.amber}40`, borderRadius:8, fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Aplicar cambio</button>
                        <button style={{ padding:"7px 14px", background:B.redDim, color:B.red, border:`1px solid ${B.red}40`, borderRadius:8, fontSize:13, fontWeight:600, cursor:"pointer", fontFamily:"inherit" }}>Reiniciar → U1</button>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          {view === "holidays" && (
            <div style={{ display:"grid", gridTemplateColumns:"1fr 340px", gap:14 }}>
              <div>
                <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, overflow:"hidden" }}>
                  <div style={{ padding:"12px 16px", borderBottom:`1px solid ${B.border}`, fontSize:13, fontWeight:700, color:B.text }}>Festivos configurados 2025–2026</div>
                  {holidays.map((h,i) => (
                    <div key={i} style={{ display:"flex", alignItems:"center", gap:12, padding:"11px 16px", borderBottom:`1px solid ${B.borderLight}` }}>
                      ...
                      <div style={{ flex:1 }}>
                        <div style={{ fontSize:13, fontWeight:500, color:B.text }}>{h.name}</div>
                        <div style={{ fontSize:12, color:B.textSec }}>{h.date}</div>
                      </div>
                      <Badge text={h.affects?"Pausa ciclo":"Sin efecto"} bg={h.affects?B.redDim:B.amberDim} color={h.affects?B.red:"#92400e"} />
                      <button onClick={() => setHolidays(hh => hh.filter((_,j)=>j!==i))} style={{ fontSize:11, padding:"3px 8px", background:B.redDim, color:B.red, border:"none", borderRadius:5, cursor:"pointer", fontFamily:"inherit" }}>✕</button>
                    </div>
                  ))}
                </div>
              </div>
              <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16, alignSelf:"start" }}>
                ...
                <div style={{ marginBottom:10 }}>
                  <label style={{ fontSize:13, color:B.textSec, display:"block", marginBottom:4 }}>Fecha</label>
                  <input type="date" value={newHoliday.date} onChange={e=>setNewHoliday(h=>({...h,date:e.target.value}))} style={{ width:"100%", padding:"8px 10px", border:`1px solid ${B.border}`, borderRadius:8, fontSize:13, background:B.bg, fontFamily:"inherit" }} />
                </div>
                <div style={{ marginBottom:10 }}>
                  <label style={{ fontSize:13, color:B.textSec, display:"block", marginBottom:4 }}>Nombre del festivo</label>
                  <input value={newHoliday.name} onChange={e=>setNewHoliday(h=>({...h,name:e.target.value}))} placeholder="Ej: Día de la Independencia" style={{ width:"100%", padding:"8px 10px", border:`1px solid ${B.border}`, borderRadius:8, fontSize:13, background:B.bg, fontFamily:"inherit" }} />
                </div>
                <div style={{ marginBottom:14 }}>
                  <label style={{ fontSize:13, color:B.textSec, display:"flex", alignItems:"center", gap:8, cursor:"pointer" }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16 }}>
                  ...
                  {XP_ACTIONS.map(a => (
                    <div key={a.key} style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 0", borderBottom:`1px solid ${B.borderLight}` }}>
                      <div style={{ flex:1, fontSize:13, color:B.text }}>{a.label}</div>
                      <div style={{ display:"flex", alignItems:"center", gap:4 }}>
                        <input type="number" value={xpValues[a.key]} onChange={e=>setXpValues(v=>({...v,[a.key]:parseInt(e.target.value)||0}))} style={{ width:60, padding:"4px 6px", border:`1px solid ${B.border}`, borderRadius:6, fontSize:13, fontWeight:700, color:B.primary, textAlign:"center", background:B.bg, fontFamily:"inherit" }} />
                ...
                <div>
                  ...
                  <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16 }}>
                    ...
                    <div style={{ fontSize:13, color:B.textSec, marginBottom:10, lineHeight:1.6 }}>El top 3 del mes recibe un descuento en su próxima mensualidad. Define el porcentaje:</div>
                    <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                ...
                <div key={int.id} style={{ background:B.white, border:`1px solid ${int.status==="connected"?B.border:B.amber}`, borderRadius:12, padding:16 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:10 }}>
                    <div style={{ display:"flex", gap:10, alignItems:"center" }}>
                      ...
                      <div>
                        <div style={{ fontSize:14, fontWeight:600, color:B.text }}>{int.name}</div>
                        <div style={{ fontSize:12, color:B.textSec, marginTop:1 }}>{int.key}</div>
                      </div>
                    </div>
                  ...
                  <div style={{ display:"flex", gap:6 }}>
                    <button style={{ flex:1, fontSize:12, padding:"6px", background:B.primaryDim, color:B.primary, border:"none", borderRadius:7, cursor:"pointer", fontWeight:600, fontFamily:"inherit" }}>{int.status==="connected"?"Reconfigurar":"Conectar"}</button>
                    {int.status==="connected" && <button style={{ fontSize:12, padding:"6px 10px", background:B.bg, color:B.textSec, border:`1px solid ${B.border}`, borderRadius:7, cursor:"pointer", fontFamily:"inherit" }}>Probar</button>}
                  </div>
                </div>
              ))}
            </div>
              ...
              {NOTIF_TEMPLATES.map((t,i) => (
                <div key={i} style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:14 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
                    <div>
                      <div style={{ fontSize:13, fontWeight:600, color:B.text }}>{t.key.replace(/_/g," ").replace(/\b\w/g,l=>l.toUpperCase())}</div>
                      <div style={{ fontSize:12, color:B.textSec, marginTop:2, display:"flex", gap:8 }}>
                        <span>Canal: <strong>{t.channel}</strong></span>
                        <span>·</span>
                        <span>Disparo: <strong>{t.trigger}</strong></span>
                      </div>
                    </div>
                    <button style={{ fontSize:12, padding:"5px 12px", background:B.primaryDim, color:B.primary, border:"none", borderRadius:7, cursor:"pointer", fontWeight:600, fontFamily:"inherit" }}>Editar plantilla</button>
                  </div>
                  <div style={{ background:B.bg, borderRadius:8, padding:"9px 12px", fontSize:13, color:B.text, fontStyle:"italic", lineHeight:1.6 }}>
                    {t.preview}
                  </div>
                </div>
              ))}
            ...
            <div>
              <div style={{ display:"flex", gap:8, marginBottom:12 }}>
                <div style={{ flex:1, display:"flex", alignItems:"center", gap:8, background:B.white, border:`1px solid ${B.border}`, borderRadius:9, padding:"7px 12px" }}>
                  ...
                  <input placeholder="Buscar en el log..." style={{ border:"none", outline:"none", fontSize:13, background:"transparent", color:B.text, flex:1, fontFamily:"inherit" }} />
                </div>
                <button style={{ padding:"7px 14px", background:B.white, border:`1px solid ${B.border}`, borderRadius:9, fontSize:13, cursor:"pointer", color:B.textSec, fontFamily:"inherit" }}>↓ Exportar log</button>
              </div>
              <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, overflow:"hidden" }}>
                  ...
                  <thead>
                    <tr style={{ background:B.bg }}>
                      {["Usuario","Acción","Detalle","Tiempo"].map(h => (
                        <th key={h} style={{ padding:"9px 12px", textAlign:"left", fontSize:11, fontWeight:600, color:B.textSec, letterSpacing:.5, textTransform:"uppercase" }}>{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
          ...
          {view === "banks" && (
            <div style={{ maxWidth:600 }}>
              <div style={{ background:B.primaryDim, border:`1px solid ${B.border}`, borderRadius:10, padding:"10px 14px", marginBottom:14, fontSize:13, color:B.primary, display:"flex", gap:8 }}>
                <i className="ti ti-shield-lock" style={{ fontSize:14, flexShrink:0 }} aria-hidden="true" />
              ...
              <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, overflow:"hidden", marginBottom:12 }}>
                {BANKS.map((b,i) => (
                  <div key={i} style={{ display:"flex", alignItems:"center", gap:12, padding:"14px 16px", borderBottom:`1px solid ${B.borderLight}`, background:!b.active?"#fafafa":"transparent" }}>
                    ...
                    <div style={{ flex:1 }}>
                      <div style={{ fontSize:14, fontWeight:600, color:b.active?B.text:B.textSec }}>{b.name}</div>
                      <div style={{ fontSize:12, color:B.textSec, marginTop:2 }}>{b.type} · {b.account} · {b.holder}</div>
                    </div>
                    <Badge text={b.active?"Activo":"Inactivo"} bg={b.active?B.greenDim:B.bg} color={b.active?"#065f46":B.textSec} />
                    <button style={{ fontSize:12, padding:"5px 10px", background:b.active?B.redDim:B.greenDim, color:b.active?B.red:"#065f46", border:"none", borderRadius:6, cursor:"pointer", fontFamily:"inherit", fontWeight:600 }}>{b.active?"Desactivar":"Activar"}</button>
                  </div>
                ))}
              </div>
              <div style={{ background:B.white, border:`1px solid ${B.border}`, borderRadius:12, padding:16 }}>
                ...
                {[
                  ...
                  <div key={f.label} style={{ marginBottom:10 }}>
                    ...
                    <input placeholder={f.ph} style={{ width:"100%", padding:"8px 12px", border:`1px solid ${B.border}`, borderRadius:8, fontSize:13, background:B.bg, fontFamily:"inherit" }} />
                  </div>
                ))}
                <div style={{ marginBottom:12 }}>
                  <label style={{ fontSize:13, color:B.textSec, display:"block", marginBottom:4 }}>Tipo de cuenta</label>
