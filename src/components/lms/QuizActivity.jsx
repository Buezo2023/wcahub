...
export function QuizActivity({ content, onComplete, completed, maxAttempts = 3 }) {
  ...
  if (phase === "taking") return (
    <div>
      ...
      {questions.map((q, qi) => (
        <div key={qi} style={{ background:"var(--bg-surface)", border:"1px solid var(--border)", borderRadius:14, padding:16, marginBottom:12 }}>
          ...
          {q.opts.map((opt, oi) => (
            <div key={oi} onClick={() => setAnswers(a=>({...a,[qi]:oi}))}
              style={{
                padding:"10px 14px", marginBottom:7, borderRadius:9, cursor:"pointer",
    ...
    <div>
      ...
      <div style={{ textAlign:"center", padding:"24px 0 20px" }}>
        <div style={{
          ...
          <div style={{ fontSize:10, color: result.passed ? G : R }}>score</div>
        </div>
      ...
      <div style={{ marginBottom:16 }}>
        {questions.map((q, qi) => {
          ...
          return (
            <div key={qi} style={{
              background:"var(--bg-surface)", borderRadius:12, padding:14, marginBottom:8,
