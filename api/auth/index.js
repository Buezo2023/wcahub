  ...
  try {
    ...
    switch (action) {
      ...
      case 'change-role': {
        ...
        if (!userId || !newRole)
          return err(res, { status: 400, message: 'userId y role son requeridos' });
        const VALID_ROLES = ['estudiante','docente','admin','super_admin','asesor_ventas','cobros','coordinadora','directivo'];
        if (!VALID_ROLES.includes(newRole))
          return err(res, { status: 400, message: `Rol inválido. Opciones: ${VALID_ROLES.join(', ')}` });
        if (newRole === 'super_admin' && actor.role !== 'super_admin')
          return err(res, { status: 403, message: 'Solo un super_admin puede promover a otro super_admin' });

        const adminCR = getSupabaseAdmin();

        // Load current profile
        const { data: profile, error: profErr } = await adminCR.from('profiles')
          .select('id, email, full_name, role').eq('id', userId).maybeSingle();
        if (profErr) return err(res, { status: 500, message: `No se pudo cargar el perfil: ${profErr.message}` });
        if (!profile) return err(res, { status: 404, message: 'Usuario no encontrado' });
        const oldRole = profile.role;

        const auditMeta = { old_role: oldRole, new_role: newRole };
        let responseMsg  = `Rol actualizado a ${newRole}`;
        let warningMsg   = null;
        ...
        if (newRole === 'estudiante') {
          ...
          const { error: p1Err } = await adminCR.from('profiles')
            .update({ role: 'estudiante', active: true }).eq('id', userId);
          if (p1Err) {
            console.error('[change-role] profiles.update failed:', { userId, error: p1Err.message });
            return err(res, { status: 500, message: `No se pudo actualizar el perfil: ${p1Err.message}` });
          }

          // Step 2: create students row if not exists
          const { data: existingStudent, error: s1Err } = await adminCR.from('students')
            .select('id').eq('profile_id', userId).maybeSingle();
          if (s1Err) {
            console.error('[change-role] students.select failed:', { userId, error: s1Err.message });
            return err(res, { status: 500, message: `Error al verificar registro de estudiante: ${s1Err.message}` });
          }
          let studentCreated = false;
          if (!existingStudent) {
            const { error: s2Err } = await adminCR.from('students')
              .insert({ profile_id: userId, level: 'A1', scholarship: false });
            if (s2Err) {
              console.error('[change-role] students.insert failed:', { userId, error: s2Err.message });
              return err(res, { status: 500, message: `No se pudo crear el registro de estudiante: ${s2Err.message}` });
            }
            studentCreated = true;
          }
          auditMeta.student_created = studentCreated;

          // Step 3: deactivate active staff rows (non-blocking if none exist)
          const { data: staffRows, error: stErr } = await adminCR.from('staff')
            .select('id').eq('profile_id', userId).eq('active', true);
          if (stErr) {
            console.error('[change-role] staff.select failed:', { userId, error: stErr.message });
            return err(res, { status: 500, message: `Error al buscar registros de staff: ${stErr.message}` });
          }
          const staffIds = (staffRows || []).map(s => s.id);
          if (staffIds.length) {
            const { error: stUpdErr } = await adminCR.from('staff')
              .update({ active: false }).in('id', staffIds);
            if (stUpdErr) {
              console.error('[change-role] staff.update failed:', { userId, error: stUpdErr.message });
              return err(res, { status: 500, message: `Error al desactivar staff: ${stUpdErr.message}` });
            }
          }
          auditMeta.staff_deactivated = staffIds.length;
          responseMsg = `Usuario convertido a estudiante.${studentCreated ? ' Registro de estudiante creado.' : ' Registro de estudiante ya existía.'}`;

        // ── B: → staff role ────────────────────────────────────────
        } else if (newRole !== 'super_admin') {

          // Warn if student has active enrollments (don't block)
          const { data: studRow } = await adminCR.from('students')
            .select('id, enrollments(id, status)').eq('profile_id', userId).maybeSingle();
          const enrList = studRow?.enrollments
            ? (Array.isArray(studRow.enrollments) ? studRow.enrollments : [studRow.enrollments])
            : [];
          const activeEnrolls = enrList.filter(e => e.status === 'active');
          if (activeEnrolls.length) {
            warningMsg = `El usuario tiene ${activeEnrolls.length} matrícula(s) activa(s). Usá "Resolver conflicto" para suspenderlas si es necesario.`;
          }

          // Step 1: update profile
          const { error: p2Err } = await adminCR.from('profiles')
            .update({ role: newRole, active: true }).eq('id', userId);
          if (p2Err) {
            console.error('[change-role] profiles.update (staff) failed:', { userId, newRole, error: p2Err.message });
            return err(res, { status: 500, message: `No se pudo actualizar el perfil: ${p2Err.message}` });
          }

          // Step 2: upsert staff row
          const STAFF_META = {
            docente:       { position: 'Docente',                department: 'Académico'      },
            coordinadora:  { position: 'Coordinadora Académica', department: 'Académico'      },
            admin:         { position: 'Administrador',          department: 'Administración' },
            cobros:        { position: 'Gestor de Cobros',       department: 'Finanzas'       },
            asesor_ventas: { position: 'Asesor de Ventas',       department: 'Ventas'         },
            directivo:     { position: 'Directivo',              department: 'Dirección'      },
          };
          const staffMeta = STAFF_META[newRole] || { position: newRole, department: 'General' };
          const { data: existingStaff, error: sf1Err } = await adminCR.from('staff')
            .select('id').eq('profile_id', userId).maybeSingle();
          if (sf1Err) {
            console.error('[change-role] staff.select (upsert) failed:', { userId, error: sf1Err.message });
            return err(res, { status: 500, message: `Error al buscar registro de staff: ${sf1Err.message}` });
          }
          if (existingStaff) {
            const { error: sf2Err } = await adminCR.from('staff')
              .update({ active: true, position: staffMeta.position, department: staffMeta.department })
              .eq('id', existingStaff.id);
            if (sf2Err) {
              console.error('[change-role] staff.update failed:', { userId, error: sf2Err.message });
              return err(res, { status: 500, message: `Error al actualizar staff: ${sf2Err.message}` });
            }
          } else {
            const { error: sf3Err } = await adminCR.from('staff').insert({
              profile_id: userId, position: staffMeta.position,
              department: staffMeta.department, active: true,
              hire_date: new Date().toISOString().slice(0, 10),
            });
            if (sf3Err) {
              console.error('[change-role] staff.insert failed:', { userId, error: sf3Err.message });
              return err(res, { status: 500, message: `Error al crear registro de staff: ${sf3Err.message}` });
            }
          }
          if (warningMsg) responseMsg += ` ⚠ ${warningMsg}`;
        }

        // Audit (non-blocking)
        await adminCR.from('audit_log').insert({
          actor_id: actor.id, action: 'changed_role', entity: 'profile',
          entity_id: userId, metadata: auditMeta,
        }).catch(() => {});

        // Force sign-out (non-blocking)
        await adminCR.auth.admin.signOut(userId, 'global').catch(() => {});

        return ok(res, { message: responseMsg, warning: warningMsg || null, old_role: oldRole, new_role: newRole });
      }
