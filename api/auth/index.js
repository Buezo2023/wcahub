...
export default async function handler(req, res) {
  ...
  try {
    ...
    case 'change-role': {
      ...
      const { data: profile } = await adminCR.from('profiles')
      ...
      if (newRole === 'estudiante') {
        // 1. Update profile
        await adminCR.from('profiles').update({ role: 'estudiante', active: true }).eq('id', userId);

        // 2. Create students row if not exists (student_code generated by DB trigger)
        const { data: existingStudent } = await adminCR.from('students')
          .select('id').eq('profile_id', userId).maybeSingle();
        let studentCreated = false;
        if (!existingStudent) {
          const { error: stErr } = await adminCR.from('students')
            .insert({ profile_id: userId, level: 'A1', scholarship: false });
          studentCreated = !stErr;
          if (stErr) console.error('[change-role] create student error:', stErr.message);
        }
        auditMeta.student_created = studentCreated;

        // 3. Deactivate any active staff rows
        const { data: staffRows } = await adminCR.from('staff')
          .select('id').eq('profile_id', userId).eq('active', true);
        const staffIds = (staffRows || []).map(s => s.id);
        if (staffIds.length) {
          await adminCR.from('staff').update({ active: false }).in('id', staffIds);
        }
      ...
      // ── B: → staff role ──────────────────────────────────────────
        ...
        // Check for active student enrollments — warn but don't block
        const { data: studRow } = await adminCR.from('students')
          .select('id, enrollments(id, status)').eq('profile_id', userId).maybeSingle();
        const studs = studRow ? [studRow] : [];
        const activeEnrolls = studs.flatMap(s => Array.isArray(s.enrollments) ? s.enrollments : (s.enrollments ? [s.enrollments] : []))
          .filter(e => e.status === 'active');
        ...
        // 1. Update profile
        await adminCR.from('profiles').update({ role: newRole, active: true }).eq('id', userId);
        ...
        const meta = STAFF_META[newRole] || { position: newRole, department: 'General' };
        const { data: existingStaff } = await adminCR.from('staff')
          .select('id').eq('profile_id', userId).maybeSingle();
        if (existingStaff) {
          await adminCR.from('staff').update({ active: true, position: meta.position, department: meta.department }).eq('id', existingStaff.id);
        } else {
          await adminCR.from('staff').insert({
            profile_id: userId, position: meta.position,
            department: meta.department, active: true,
      ...
      // ── Force sign-out so next login reads new role ───────────────
